...
class TransformerConfig(ModelParallelConfig):
    ...
    # Grouter Related
    ##################
    moe_use_grouter: bool = False
    """Set True to use grouter"""

    grouter_output_logits: bool = False
    """Ask grouter output logits"""

    dynamic_act_threshold: float = 0.0
    """The activation threshold"""

    dynamic_act: bool = False
    "Trun on dynamic activation"

    fp32_grouter: bool = False
    """Use FP32 grouter"""

    grouter_checkpoint_path: str = None
    """The path to load grouter checkpoint. If not use single grouter, one should take
    grouter_checkpoint_path as a prefix and name the real checkpint with their layer num.
    We only support grouter number equal to layers number nowaday"""

    grouter_bias_checkpoint_path: str = None
    """The path to load grouter bias"""

    use_single_grouter: bool = True
    """Only use one grouter to guide all moe layers"""

    grouter_convert: bool = False
    """Convert grouter to different activation / total experts distribution"""

    use_grouter_weight: bool = False
    """Use grouter logits to generate scores"""

    grouter_scoring_fun: str = 'softmax'
    """Assign the scoring function for grouter"""

    grouter_model_config_path: str = None
    """The path for grouter structure config"""

    grouter_structure_type: str = None
    """The structuer tupe for grouter"""

    grouter_enable_distillation: bool = False
    """Enable Grouter distillation training"""
    
    grouter_type: str = "linear"
    """Type of Grouter architecture"""
    
    grouter_distillation_temperature: float = 2.0
    """Temperature for distillation"""
    
    grouter_distillation_lr: float = 0.001
    """Learning rate for Grouter distillation"""
    
    grouter_moe_layer_start: int = 0
    """Start layer index for MoE layers"""
    
    grouter_moe_layer_end: int = 1
    """End layer index for MoE layers"""

    grouter_init_seed: int = 1234
    """The random seed for grouter parameter initialization"""
    
    grouter_checkpoint_dir: str = None
    """Directory to save Grouter checkpoints"""
    
    grouter_checkpoint_interval: int = 1000
    """Save Grouter checkpoint every N steps"""
    
    grouter_resume_from: str = None
    """Path to Grouter checkpoint to resume from"""
    
    grouter_config_path: str = None
    """Path to JSON file containing Grouter configuration parameters"""

    distillation_warmup_iters: int = None
    """The warmup step for distillation"""

    distillation_train_iters: int = None
    """The train step for distillation"""

    distillation_gradient_accumulation_steps: int = None
    """The gradient accumulation steps for distillation"""

    use_grouter_dataset: bool = False
    """Use the dataset prebuilded by grouter"""

    grouter_expert_placement_path: str = None
    """Expert placement configuration JSON file path"""
    
    grouter_enable_global_migration: bool = False
    """Enable global expert migration at training start"""
    
    grouter_migration_strategy: str = "allreduce"
    """Expert migration communication strategy. Choices: allreduce, p2p"""
    
    grouter_compression_ratio: float = 1
    """Parameter compression ratio for expert migration (0.0-1.0)"""
    
    grouter_migration_timeout: float = 30.0
    """Migration timeout in seconds"""
    
    grouter_migration_max_retries: int = 3
    """Maximum number of migration retries"""
    
    grouter_migration_verbose: bool = False
    """Enable detailed migration logs"""
    
    expert_migration_steps_config_path: str = None
    """Expert migration steps configuration JSON file path"""

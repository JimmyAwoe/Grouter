    ...
    def load(self, sharded_state_dict: ShardedStateDict, checkpoint_dir: Path) -> StateDict:
        ...
        for sh_ten in nested_values(sharded_state_dict):
            if is_nd_flattened_tensor(sh_ten) and sh_ten.key not in reformulation_metadata:
                ...
                break
        args = get_args()
        if args.grouter_allow_partial_load:
        ...
        # For partial loading: map model keys to checkpoint keys
        if args.grouter_allow_partial_load:
            import re
            ckpt_metadata = fsr.read_metadata()
            ckpt_keys = set(ckpt_metadata.state_dict_metadata.keys())
            
            # we should revise the name in each dict to ensure right map
            filtered_dict = {}
            key_mapping = {}
            
            for key, value in pyt_state_dict.items():
                if key in ckpt_keys:
                    filtered_dict[key] = value
                    continue
                
                # Pattern matching 
                # In qwen ckpt loading, there are two pattern
                # 1) expert relate module: xxx/shard_{layer}.{expert}_{num_layers}.{num_experts}
                # 2) non expert relate module: xxx/shard_{layer}_{num_layers}
                
                expert_match = re.match(r'(.+)/shard_(\d+)\.(\d+)_(\d+)\.(\d+)$', key)
                non_expert_match = re.match(r'(.+)/shard_(\d+)_(\d+)$', key)
                if expert_match:
                    base_key, layer_idx, expert_idx, _, num_experts = expert_match.groups()
                    # match in ckpt
                    pattern = re.compile(rf'^{re.escape(base_key)}/shard_{layer_idx}\.{expert_idx}_\d+\.{num_experts}$')
                    found_match = None
                    for ck in ckpt_keys:
                        if pattern.match(ck):
                            found_match = ck
                    ...
                    if found_match:
                        if isinstance(value, TorchShardedTensor):
                            value.key = found_match
                        filtered_dict[found_match] = value
                        key_mapping[key] = found_match
                    else:
                        raise RuntimeError(f"The {key} in expert pattern do not fint matching module")
                elif non_expert_match:
                    base_key, layer_idx, _= non_expert_match.groups()
                    pattern = re.compile(rf'^{re.escape(base_key)}/shard_{layer_idx}_\d+$')
                    found_match = None
                    for ck in ckpt_keys:
                        if pattern.match(ck):
                            found_match = ck
                    ...
                    if found_match:
                        ...
                        raise RuntimeError(f"The {key} in non-expert pattern do not fint matching module")
                else:
                    raise RuntimeError(f"The {key} has not match any pattern")
            
            pyt_state_dict = filtered_dict
            
            if key_mapping:
                new_rename_mapping = {}
                for old_key, values in rename_mapping.items():
                    new_key = key_mapping.get(old_key, old_key)
                    new_rename_mapping[new_key] = values
                rename_mapping = new_rename_mapping

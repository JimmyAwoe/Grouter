...
class GPTDataset(MegatronDataset):
    ...
    def _build_document_sample_shuffle_indices(
        ...
        if not path_to_cache or (
            ...
            if not hasattr(self.config, 'no_document_idx_shuffle'):
                self.config.no_document_idx_shuffle = False
            document_index = _build_document_index(
                self.indices, num_epochs, numpy_random_state, separate_final_epoch, self.config.no_document_idx_shuffle
            )
            ...
            if not hasattr(self.config, "no_shuffle_effect"):
                self.config.no_shuffle_effect = False
            if separate_final_epoch:
                shuffle_index = _build_shuffle_index(
                    num_samples_sans_final_epoch, sample_index.shape[0] - 1, numpy_random_state, self.config.no_shuffle_effect
                )
            else:
                shuffle_index = _build_shuffle_index(
                    sample_index.shape[0] - 1, sample_index.shape[0] - 1, numpy_random_state, self.config.no_shuffle_effect
                )
...
def _build_document_index(
    ...
    Args:
        ...
        no_shuffle (bool): Whether to ban shuffle
    ...
    doc_idx_first = _build_document_index(documents, num_epochs - 1, numpy_random_state, False, no_shuffle)
    doc_idx_last = _build_document_index(documents, 1, numpy_random_state, False, no_shuffle)
    return numpy.concatenate((doc_idx_first, doc_idx_last))


def _build_shuffle_index(
    num_samples: int, total_size: int, numpy_random_state: numpy.random.RandomState, no_shuffle: bool = False
) -> numpy.ndarray:
    ...
    Args:
        ...
        numpy_random_state (numpy.random.RandomState): The NumPy random state
        no_shuffle: Remove shuffle effect

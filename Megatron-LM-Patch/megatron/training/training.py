...
def get_model(model_provider_func, model_type=ModelType.encoder_or_decoder, wrap_with_ddp=True):
    ...
    if args.fp16 or args.bf16:
        ...
        # load grouter here to ensure fp32
        if config.fp32_grouter:
            for param in model[0].module.grouter.parameters():
                param.data = param.data.to(dtype=torch.float32)
            current_device = f"cuda:{torch.cuda.current_device()}"
            checkpoint = torch.load(config.grouter_checkpoint_path, map_location=current_device)
            model[0].module.grouter.load_state_dict(checkpoint)
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config, forward_backward_func):
    ...
    while rerun_state_machine.should_run_forward_backward(data_iterator):
        ...
        losses_reduced = forward_backward_func(
            ...
            forward_only=args.grouter_enable_distillation,
    ...
    # Update parameters.
    # Skip optimizer step if in distillation mode (forward_only)
    if not args.grouter_enable_distillation:
        timers('optimizer', log_level=1).start(barrier=args.barrier_with_L1_time)
        update_successful, grad_norm, num_zeros_in_grad = optimizer.step()
        timers('optimizer').stop()
    else:
        # In distillation mode, no parameter updates
...
def training_log(
    ...
    if writer and (iteration % args.tensorboard_log_interval == 0):
        ...
        if grouter_dynamic_sparsity is not None:
            writer.add_scalar('grouter-sparsity', grouter_dynamic_sparsity, iteration)
            writer.add_scalar('grouter-sparsity vs samples', grouter_dynamic_sparsity, args.consumed_train_samples)
            if wandb_writer:
                wandb_writer.log({'grouter-sparisty': grouter_dynamic_sparsity}, iteration)
    ...
    if iteration % args.log_interval == 0:
        ...
        if grouter_dynamic_sparsity is not None:
            log_string += f' grouter sparsity ratio: {grouter_dynamic_sparsity:.3f} |'
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        if config.dynamic_act and args.dynamic_act_threshold < config.moe_router_topk_scaling_factor:
            config.sparsity.zero_()
        ...
        grouter_dynamic_sparsity = None
        if config.dynamic_act and args.dynamic_act_threshold < config.moe_router_topk_scaling_factor:
            # calculate the average sparsity
            config.sparsity /= num_microbatches
            torch.distributed.all_reduce(config.sparsity, 
                                         group=mpu.get_data_parallel_group(), 
                                         op=torch.distributed.ReduceOp.AVG)
            torch.distributed.all_reduce(config.sparsity, 
                                         group=mpu.get_pipeline_model_parallel_group(), 
                                         op=torch.distributed.ReduceOp.SUM)
            grouter_dynamic_sparsity = config.sparsity.item()
...
def build_train_valid_test_data_iterators(build_train_valid_test_datasets_provider):
    ...
    if args.use_grouter_dataset:
        if train_dataloader is not None:
            train_data_iterator = _get_iterator(dl_type, iter(train_dataloader))
        else:
            train_data_iterator = None

        if valid_dataloader is not None:
            valid_data_iterator = _get_iterator(dl_type, iter(valid_dataloader))
        else:
            valid_data_iterator = None

        if test_dataloader is not None:
            test_data_iterator = _get_iterator(dl_type, iter(test_dataloader))
        else:

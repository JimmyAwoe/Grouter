...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, load_arg='load', strict=True,
    ...
    if ckpt_format == "torch_dist":
        if args.grouter_enable_distillation and 'args' not in state_dict.keys():
            import copy
            state_dict['args'] = copy.deepcopy(args)
            # Megatron-Bridge set tp-pp as 1-1 defaultly
            state_dict['args'].tensor_model_parallel_size = 1
            state_dict['args'].pipeline_model_parallel_size = 1
            state_dict['args'].context_parallel_size = 1
            state_dict['args'].data_parallel_size = 0
    ...
    def load_model_state_dict(module, state_dict, strict: bool):
        ...
        try:
            # Grouter parameter need not load
            if hasattr(module.module.module, 'grouter'):
                state_dict = {k: v for k, v in state_dict.items() if "grouter" not in k}
                original_grouter = module.module.module.grouter
                del module.module.module.grouter
                module.load_state_dict(state_dict, strict=strict)
                module.module.module.grouter = original_grouter
            else:
                module.load_state_dict(state_dict, strict=strict)
        except Exception as e:
            if strict:
